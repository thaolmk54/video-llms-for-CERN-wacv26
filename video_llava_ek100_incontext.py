...
with open("abductiveEk100/post_processed_test_data_instances.json") as f:
    ...
    for instance in tqdm(data, total=len(data)):
        ...
        with open("abductiveEk100/post_processed_train_data_instances.json") as f_ict:
            ...
            for instance_ict in picked_instances:
                ...
                if in_context_prompt == "":
                    in_context_prompt += f"USER: Given a sequence of premise events include {event_list}, what is the cause event of the event {target_event}? Choose a correct answer within the premise events. ASSISTANT: {explaination_event}. "
                else:
                    in_context_prompt += f"USER: Given a sequence of premise events include {event_list}, what is the cause event of the event {target_event}? Choose a correct answer within the premise events. ASSISTANT: {explaination_event}. "
        # now run the test
        ...
        prompt = in_context_prompt + f" USER: <video>Given a sequence of premise events include {event_list}, what is the cause event of the event {target_event}? Choose a correct answer within the premise events. ASSISTANT:"
        # print("prompt:", prompt)
...
# <video> at the beginning of incontext prompt: 34.50%
# <video> at the beginning of actual test prompt: 25.58%